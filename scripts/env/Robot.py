...
from humanoid_robot.srv import ResetJoints
...
class Robot:    
    ...
    def apply_action(self, action):
        ...
        self.trajectory_node.set_target_trajectory(self.joints, mapped_actions, time_to_end_sec=0.1)
    ...
    def _reset_pose(self):
        ...
        request = self._create_pose_reset_request()
        self._send_pose_reset_request(client, request)

    def _reset_joints(self):
        client = self._get_reset_client()
        request = self._create_joint_reset_request()
        self._send_joint_reset_request(client, request)
    ...
    def _create_pose_reset_request(self):
    ...
    def _send_pose_reset_request(self, client, request):
    ...
    def _get_reset_client(self):
        client = self.env.create_client(ResetJoints, '/reset_joints')
        while not client.wait_for_service(timeout_sec=1.0):
            self.env.get_logger().warn("Waiting for /reset_joints...")

        return client

    def _create_joint_reset_request(self):
        request = ResetJoints.Request()
        request.joint_names = self.joints
        request.positions = self.default_joint_positions

        return request

    def _send_joint_reset_request(self, client, request):
        future = client.call_async(request)
        rclpy.spin_until_future_complete(self.env, future)

        if future.result() is None:
            self.env.get_logger().error("Failed to reset robot's joints.")
